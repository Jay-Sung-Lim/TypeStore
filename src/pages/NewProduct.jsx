    ...
    uploadImage(file) //
      .then((url) => {
        addProduct.mutate(
          ...
          {
            onSuccess: () => {
              setSuccess('New product is registered successfully');
              setTimeout(() => {
                setSuccess(null);
              }, 4000);
            },
  ...
  return (
    <section className='w-full text-center'>
      ...
      <form className='flex flex-col px-12' onSubmit={handleSubmit}>
        ...
        <input type='text' name='options' value={product.options ?? ''} placeholder='Options (separated by ,)' required onChange={handleChange} />
        <Button text={isUploading ? 'Uploading...' : 'Register'} disabled={isUploading} />
      </form>
    </section>
  );
}
